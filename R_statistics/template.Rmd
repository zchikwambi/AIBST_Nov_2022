...
> Answer: R picks random values around the mean and within the standard derviation, amking it difficult to return the same mean and sd value provided.
...
```{r}
quantile(y, seq(0, 1, length.out=10))
```
...
abline(v=median(y), col="red", lty="dashed")
abline(v=mean(y)-sd(y), col="blue")
abline(v=mean(y)+sd(y), col="blue")
abline(v=median(y)+mad(y), col="blue", lty="dashed")
abline(v=median(y)-mad(y), col="blue", lty="dashed")
...
```{r}
#pnorm(seq(-5, 5, 0.1))
plot(seq(-5, 5, 0.1), pnorm(seq(-5, 5, 0.1)))
...
```{r}
#plot(seq(-5, 5, 0.1), pnorm(seq(-5, 5, 0.1))>2)
...
```{r}
T1<-t.test(gene_exprs~group, df)
```


```{r}
groupA<-df$gene_exprs[df$group=="groupA"]
groupB<-df$gene_exprs[df$group=="groupB"]
t.test(groupA, groupB)
```

**What is the p-value?**

> Answer:p-value = 0.0182
> 
