...
```{r}
dds_final_filtered<-dds[rowSums(assays(dds_final)$count) >0, ]
dds_final_filtered
```

# remove unwanted samples 
```{r}
dds_final_filtered_subset <- dds_final_filtered[,1:6]
```
...
plotCounts(dds_final_filtered_subset,
           gene = "ENSMUSG00000041779", intgroup = "cell_type")
```

```{r}
# Plot normalised counts for gene ENSMUSG00000041779
plotCounts(dds_final_filtered_subset,
           gene = "ENSMUSG00000041779", returnData = T, intgroup = "cell_type")
```
...
res_contrast<-results(dds_final_filtered_DE, alpha = 0.05,contrast = c("cell_type", "CD8","CD4"))
res_contrast
```
```{r}
res_contrast_rev<-results(dds_final_filtered_DE, alpha = 0.05,contrast = c("cell_type", "CD4","CD8"))
res_contrast_rev
```
```{r}
#Compare(res_contrast$pvalue, res_contrast$padj)
#, summary(res_contrast_rev), fun = "==")
  
print(summary(res_contrast), summary(res_contrast_rev))
```
...
```{r}
shrunkenLFC_dds_results_obj<-lfcShrink(
  dds=dds_final_filtered_DE,
  coef = "cell_type_CD8_vs_CD4",
  res= res,
  type = "apeglm",
  lfcThreshold = 0,
  svalue = FALSE,
...
summary(shrunkenLFC_dds_results_obj) 
...
gene_symbols<-mapIds(x=org.Mm.eg.db, keys = rownames(shrunkenLFC_dds_results_df), column = "SYMBOL", keytype = "ENSEMBL")
gene_symbols
...
shrunkenLFC_dds_results_df$gene_symbol<-gene_symbols
shrunkenLFC_dds_results_df
```
